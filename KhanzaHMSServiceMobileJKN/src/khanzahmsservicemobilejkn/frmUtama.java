...
public class frmUtama extends javax.swing.JFrame {
    ...
    private String requestJson, URL = "", utc = "", link = "", datajam = "", nol_jam = "", nol_menit = "", nol_detik = "", jam = "", menit = "", datataskid = "",
                ...
                if (detik.equals("01") && ((nilai_menit % 10) == 0)) {
                    ...
                    try {
                        ...
                        LocalDateTime currentTime = LocalDateTime.now();
                                LocalDateTime taskid1 = currentTime.minus(10,ChronoUnit.MINUTES);
                                LocalDateTime taskid2 = currentTime.minus(5,ChronoUnit.MINUTES);
                                long result1 = taskid1.atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();
                                long result2 = taskid2.atZone(java.time.ZoneId.systemDefault()).toInstant().toEpochMilli();
                                System.out.println("milis 1 : "+result1);
                                System.out.println("milis 2 : "+result2);
                                Date date1 = new Date(result1);
                                Date date2 = new Date(result2);
...
//                                System.out.println("date 2: "+date2);
                        ...
                        try {
                            rs = ps.executeQuery();
                            while (rs.next()) { 
                               //Running Task ID 1 MJKN
                               TeksArea.append("Menjalankan WS ADMISI 1 Antrian Mobile JKN\n");
                               ps2 = koneksi.prepareStatement("select reg_periksa.no_reg,reg_periksa.no_rawat,reg_periksa.tgl_registrasi,reg_periksa.kd_dokter,dokter.nm_dokter,reg_periksa.kd_poli,poliklinik.nm_poli,reg_periksa.stts_daftar,reg_periksa.no_rkm_medis,reg_periksa.kd_pj "+
                               ...
                               rs2=ps2.executeQuery();
                                while (rs2.next()) {
                                    ps3=koneksi.prepareStatement("select * from jadwal where jadwal.hari_kerja=? and jadwal.kd_dokter=? and jadwal.kd_poli=?");
                                    try {
                                        ps3.setString(1, hari);
                                        ps3.setString(2,rs2.getString("kd_dokter"));
                                        ps3.setString(3,rs2.getString("kd_poli"));
                                        rs3=ps3.executeQuery();
                                        if(rs3.next()){
                                            kodedokter=Sequel.cariIsi("select maping_dokter_dpjpvclaim.kd_dokter_bpjs from maping_dokter_dpjpvclaim where maping_dokter_dpjpvclaim.kd_dokter=?",rs3.getString("kd_dokter"));
                                            kodepoli=Sequel.cariIsi("select maping_poli_bpjs.kd_poli_bpjs from maping_poli_bpjs where maping_poli_bpjs.kd_poli_rs=?",rs3.getString("kd_poli"));
                                        if((!kodedokter.equals(""))&&(!kodepoli.equals(""))){
                                            datajam=Sequel.cariIsi("select if(concat(reg_periksa.tgl_registrasi,' ',reg_periksa.jam_reg)>concat('"+rs2.getString("tgl_registrasi")+"',' ','"+rs3.getString("jam_mulai")+"'), "+
                                                            ...
                                                            "STR_TO_DATE(DATE_ADD(concat('"+rs2.getString("tgl_registrasi")+"',' ','"+rs3.getString("jam_mulai")+"'), INTERVAL (CAST(RAND()*(240-180+1)-180 AS INT)) SECOND), '%Y-%m-%d %H:%i:%s')) as task1 "+
                                                            "from reg_periksa where kd_poli<>'IGDK' and stts_daftar='Baru' and no_rawat=?",rs2.getString("no_rawat"));
                                            datataskid=Sequel.cariIsi("select * from referensi_mobilejkn_bpjs_taskid where taskid='1' and no_rawat=?",rs2.getString("no_rawat"));
                                            if(!datajam.equals("")){
                                                if(datataskid.equals("")){
                                                    if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{rs.getString("no_rawat"),"1",datajam})==true){
                                                        ...
                                                        try {     
                                                            ...
                                                            requestEntity = new HttpEntity(requestJson,headers);
                                                            URL = link+"/antrean/updatewaktu";	
                                                            System.out.println("Task ID 1 MJKN - URL : "+URL);
                                                            //System.out.println(api.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                                                            root = mapper.readTree(api.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                                                            nameNode = root.path("metadata");
                                                            if(!nameNode.path("code").asText().equals("200")){
                                                ...
                                                if(datataskid.equals("")){
                                                    if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{rs.getString("no_rawat"),"2",datajam})==true){
                                                        ...
                                                        try {     
                                                            ...
                                                            requestEntity = new HttpEntity(requestJson,headers);
                                                            URL = link+"/antrean/updatewaktu";	
                                                            System.out.println("Task ID 2 MJKN - URL : "+URL);
                                                            //System.out.println(api.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                                                            root = mapper.readTree(api.getRest().exchange(URL, HttpMethod.POST, requestEntity, String.class).getBody());
                                                            nameNode = root.path("metadata");
                                                            if(!nameNode.path("code").asText().equals("200")){
