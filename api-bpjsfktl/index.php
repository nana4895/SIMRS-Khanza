                        ...
                        switch ($url[0]) {
                                    ...
                                    if (empty($decode['nomorkartu'])){ 
                                        ...
                                        if(empty($kdpoli)) { 
                                            ...
                                            if(empty($jadwal['kuota'])) {
                                                $response = array(
                                                    'metadata' => array(
                                                        'message' => 'Maaf, Booking Periksa Ditutup Karena Kuota Terpenuhi',
                                                        'code' => 201
                                                    )
                                                );
                                                http_response_code(201);
                                            }else{
												$px=cekpasien2($decode['nik']);
                                                if(empty($px['no_ktp'])){ 
                                                ...
                                                }elseif($px['no_peserta']!=$decode['nomorkartu']){
													$response = array(
                                                        'metadata' => array(
                                                            'message' =>  'Nomer BPJS Anda tidak terdaftar Pada RS Kami, Harap menghubungi 081331498900 Untuk melengkapi data Anda' ,
                                                            'code' => 202
                                                        )
                                                    ); 
                                                    http_response_code(202);
                                                        ...
                                                        if($sudahdaftar>0){
                                                            ...
                                                            if($interval<0){
                                                                ...
                                                                if ($sisakuota < $jadwal['kuota']) {
																	...
																	$datapeserta     = cekpasien2($decode['nik']);
                                                                    ...
                                                                    if (empty($datapeserta ['no_rkm_medis'])){
                                                                        $nomer_rm = $decode['norm'];
                                                                    }else{
                                                                        $nomer_rm = $datapeserta['no_rkm_medis'];
                                                                    }

                                                                    $execute=date("Y-m-d H:i:s");
                                                                    $date = date("Y-m-d");
                                                                    $waktu = date("H:i:s");
                                                                    $waktu_kunjungan = $decode['tanggalperiksa'].' '.$waktu;
                                                                    $insertbooking_reg = bukaquery4("INSERT INTO booking_registrasi VALUES 
                                                                    ('$date','$waktu','$nomer_rm','$decode[tanggalperiksa]','$kddokter','$kdpoli','$noReg','BPJ',1,'$execute','Terdaftar')");

                                                                    
                                                                    $querybooking = bukaquery4("insert into referensi_mobilejkn_bpjs values('$nobooking','$no_rawat', '$decode[nomorkartu]', '$decode[nik]', '$decode[nohp]', '$decode[kodepoli]','$statusdaftar', '$nomer_rm', '$decode[tanggalperiksa]', '$decode[kodedokter]', '$decode[jampraktek]', '$jeniskunjungan', '$decode[nomorreferensi]','".$kdpoli."-".$noReg."','$noReg','".(strtotime($jadwal['jam_mulai'].'+'.$dilayani.' minute')* 1000)."','".($jadwal['kuota']-$sisakuota-1)."','$jadwal[kuota]','".($jadwal['kuota']-$sisakuota-1)."','$jadwal[kuota]','Belum','0000-00-00 00:00:00','Belum')");
                                                                    $query_ = bukaquery2("INSERT INTO trackersql VALUES ('$execute','insert into referensi_mobilejkn_bpjs values(no_booking=$nobooking, no_rawat=$no_rawat, id_dokter=$kddokter, no_rm=$nomer_rm, kd_poli=$kdpoli)','Pasien MJKN')");
                                                                    if ($querybooking) {
                                                                        $query = bukaquery2("insert into reg_periksa values('$noReg', '$no_rawat', '$decode[tanggalperiksa]',current_time(), '$kddokter', '$nomer_rm', '$kdpoli', '$datapeserta[namakeluarga]', '$datapeserta[alamatpj], $datapeserta[kelurahanpj], $datapeserta[kecamatanpj], $datapeserta[kabupatenpj], $datapeserta[propinsipj]', '$datapeserta[keluarga]', '".getOne2("select registrasilama from poliklinik where kd_poli='$kdpoli'")."', 'Belum','".str_replace("0","Lama",str_replace("1","Baru",$statusdaftar))."','Ralan', '".CARABAYAR."', '$umur','$sttsumur','Belum Bayar', '$statuspoli')");
                                                                        if ($query) {
                                                                            $response = array(
                                                                                'response' => array(
                                                                                    ...
                                                                                    'keterangan'=> 'Pasien harap datang 15 menit lebih awal dan konfirmasi registrasi ke Pendaftaran.'
                                                                                ),
                                                                                'metadata' => array(
                                                                                    'message' => 'Ok',
                                                                                    'code' => 200
                            ...
                            case "checkinantrean":
                                ...
                                if((!empty($header['x-token'])) && (USERNAME==$header['x-username']) && (cektoken($header['x-token'])=='true')){
                                    ...
                                    @$jamCheckin = date("H:i:s", ($decode['waktu']/1000));
                                    if(empty($decode['kodebooking'])) { 
                                        ...
                                        $booking = fetch_array(bukaquery2("select nobooking,tanggalperiksa,status,validasi,left(jampraktek,5) as jampraktek, kodepoli from referensi_mobilejkn_bpjs where nobooking='$decode[kodebooking]'"));
                                        if(empty($booking['status'])) {
                                            ...
                                            if($booking['status']=='Batal'){
                                                ...
                                                $ambilJadwalRS = fetch_array(bukaquery4("SELECT jdwl.hari_kerja,jdwl.jam_mulai,jdwl.jam_selesai,mapping.kd_poli_bpjs 
                                                                            FROM jadwal jdwl
                                                                            INNER JOIN maping_poli_bpjs mapping ON jdwl.kd_poli = mapping.kd_poli_rs
                                                                            WHERE mapping.kd_poli_bpjs LIKE '".$booking['kodepoli']."'"." ".
                                                                            "AND jdwl.hari_kerja LIKE '".$hari."'"));

                                                // $kondisiAwal  = getOne2("select TIMESTAMPDIFF(MINUTE,'$booking[tanggalperiksa] $ambilJadwalRS[jam_selesai]','$tanggalchekcin') AS difference");
                                                // $kondisiAkhir  = getOne2("select TIMESTAMPDIFF(MINUTE,'$booking[tanggalperiksa] $ambilJadwalRS[jam_selesai]','$tanggalchekcin') AS difference");

                                                $interval  = fetch_array(bukaquery2("SELECT TIMESTAMPDIFF(MINUTE,'$booking[tanggalperiksa] $ambilJadwalRS[jam_mulai]','$tanggalchekcin') AS PoliBuka, 
                                                                        TIMESTAMPDIFF(MINUTE,'$booking[tanggalperiksa] $ambilJadwalRS[jam_selesai]','$tanggalchekcin') AS PoliTutup"));
                                                //logic untuk checkin sebelum buka pelayanan poli
                                                                                               
                                                /* 
                                                if($interval['PoliBuka']>=-60 && $interval['PoliBuka']>=0){
                                                    $response = array(
                                                        'metadata' => array(
                                                            'message' => "Chekin Anda sudah expired pada jam ".$jamCheckin." . Silahkan konfirmasi ke loket pendaftaran",
                                                            'code' => 201
                                                        )
                                                    );  
                                                    http_response_code(201);
                                                ...
                                                */
                                                ...
                                                //_____Logic Untuk bisa checkIn 1 Jam sebelum poli tutup_______
                                                // $kondisi = false;
                                                if($interval['PoliBuka'] >= -60 && $interval['PoliTutup'] <= -60){
                                                    $update=bukaquery2("update referensi_mobilejkn_bpjs set status='Checkin',validasi=now() where nobooking='$decode[kodebooking]'");
                                                    if($update){
                                                        $response = array(
                                                            'metadata' => array(
                                                                'message' => 'Ok '. $jamCheckin ,
                                                                'code' => 200
                                                            )
                                                        );
                                                        http_response_code(200);
                                                    ...
                                                    if($interval['PoliBuka'] < -60 ){
                                                        $response = array(
                                                            'metadata' => array(
                                                                'message' => 'Check In Peserta hanya bisa dilakukan 1 jam sebelum poli buka, Silahkan konfirmasi ke Loket Pendaftaran.',
                                                                'code' => 201
                                                            )
                                                        );  
                                                        http_response_code(201);
                                                        ...
                                                        $response = array(
                                                            'metadata' => array(
                                                                'message' => "Chekin Anda sudah expired ".$jamCheckin." . Silahkan konfirmasi ke loket pendaftaran",
                                                                'code' => 201
                                                            )
                                                        );  
                                                        http_response_code(201);
                                    ...
                                    if(empty($decode['kodebooking'])) { 
                                        ...
                                        if(empty($booking['status'])) {
                                            ...
                                            if($booking['status']=='Batal'){
                                                ...
                                                $batal         = bukaquery4("delete from reg_periksa where no_rawat='$booking[no_rawat]'");
                                                $batalBooking_reg = bukaquery4("DELETE FROM booking_registrasi WHERE tanggal_periksa = '$booking[tanggalperiksa]' AND no_rkm_medis = '$booking[norm]'");
                                                if($batal){
                                                    $response = array(
                                                        'metadata' => array(
                                                            'message' => 'Ok',
